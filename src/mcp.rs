...
use rmcp::{
    ...
    model::{CallToolResult, Content, Implementation, InitializeResult,
...
impl AuraMcpServer {
    ...
    #[tool(description = "Store a new memory. Levels: working (hours), decisions (days), domain (weeks), identity (months+). Auto-detects novel info and boosts level.")]
    ...
    #[tool(description = "Store a code snippet at DOMAIN level with language metadata and syntax highlighting in recall.")]
    async fn store_code(&self, Parameters(p): Parameters<StoreCodeParams>) -> Result<CallToolResult, McpError> {
    ...
    #[tool(description = "Store a decision with reasoning and rejected alternatives at DECISIONS level.")]
    async fn store_decision(&self, Parameters(p): Parameters<StoreDecisionParams>) -> Result<CallToolResult, McpError> {
        let mut content = format!("DECISION: {}", p.decision);
        if let Some(ref r) = p.reasoning {
            if !r.is_empty() { content.push_str(&format!("\nREASONING: {}", r)); }
        }
        if let Some(ref a) = p.alternatives {
            if !a.is_empty() { content.push_str(&format!("\nALTERNATIVES: {}", a.join(", "))); }
        }
        let mut tags = p.tags.unwrap_or_default();
    ...
    #[tool(description = "Get proactive insights about memory health. Detects decay risks, promotion candidates, clusters, conflicts, and trends.")]
    ...
    #[tool(description = "Merge similar memory records (85%+ similarity) to reduce bloat. Call periodically for hygiene.")]
    async fn consolidate(&self) -> Result<CallToolResult, McpError> {
        let result = self.brain.consolidate()
            .map_err(|e| err(e.to_string()))?;
        ...
        ServerInfo {
            ...
            instructions: Some(
                "Aura is a cognitive memory layer for AI agents. \
                 ...
                 Use 'insights' to check memory health.".into()
            ),
        }
    }
